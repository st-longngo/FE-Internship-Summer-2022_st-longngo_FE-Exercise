import { getData, keyList, renderCartNumberOfListProduct, products, formatFixed, setData } from "./common";
...
function renderProduct() {
  var listProduct = products.map((item :any) => {
...
function addCart(id: string) : void{
  ...
  const productCart = cart.find((item: any) => item.id === id);
  if (!productCart) {
  ...
  setData(keyList.cart, cart.sort((a: any, b: any) => a.id - b.id))
  renderCartNumberOfListProduct();
}
